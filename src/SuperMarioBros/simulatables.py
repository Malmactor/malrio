...
def seg_aabb_collision_resolution(box_center, radius, start, direction, padding=(0, 0), config=None):
    ...
    near_time_vec = (box_center - sign_vec * (radius + padding) - start) / (direction + epsilon)
    far_time_vec = (box_center + sign_vec * (radius + padding) - start) / (direction + epsilon)
    ...
    collision["delta"] = collision["time"] * direction
...
class CollidableAABB:
    ...
    def static_collide(self, collidable):
        d = collidable.center - self.center
        ...
        if p[0] < p[1]:
            ...
            collision["position"] = np.array([self.center[0] + self.radius[0] * sign_vec[0], collidable.center[1]],
                                             dtype=self.dtype)
        else:
            sign_vec = np.sign(d)
            collision["delta"] = np.array([0, p[1] * sign_vec[1]], dtype=self.dtype)
            collision["normal"] = np.array([0, sign_vec[1]], dtype=self.dtype)
            collision["position"] = np.array([collidable.center[0], self.center[1] + self.radius[1] * sign_vec[1]],
                                             dtype=self.dtype)
    ...
    def collide(self, collidable_rigid):
        ...
        if np.all(velocity <= self.epsilon):
            collision["position"] = collidable_rigid.center
